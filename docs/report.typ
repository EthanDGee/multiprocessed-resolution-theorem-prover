...
The `MultiThreadedResolver` acts as the manager, It starts/stops threads and determines when the proof is complete. When the `prove` method is called, it initializes a thread pool with one `ProverThread` per available CPU core. It then enters a monitoring loop that checks for one of two termination conditions: the creation of the empty clause, (I did think of a silly little joke I thought you might like while writing this paper "Who comes down the chimney to give all the good mathematicians proofs for Christmas? The empty clause". It probably needs some work, but I thought you would like it) or the system has reached saturation (all clauses have been marked resolved, and no threads are working). It relies on the use of atomic variables to monitor this: `solutionFound` (an `AtomicBoolean`) and `activeWorkers` (an `AtomicInteger`). It also uses an additional `hasUnresolvedClauses()` given by the database class. When a proof is completed, the `MultiThreadedResolver` interrupts all worker threads to ensure a clean shutdown and then waits for them to terminate. Finally it returns `solutionFound`.

The `ProverThread` class contains the logic for the individual worker threads, and handles the actual resolution. Each thread runs a main loop that continues until a solution is found or it is interrupted by the coordinator. While running, it first requests a batch of unresolved clauses from the database by calling `database.getUnresolvedClauses()`. This has integrated blocking so that the thread waits until work is available. Once a batch is acquired, the thread signals to the coordinator that it is active by incrementing `activeWorkers` and begins the resolution process. It starts by looping in batches back to front from the last index in the unresolved batch to the start of the database. This back to front method of unification comes with the added benefit of prioritizing clause combinations that are the most likely to lead to new resolutions. All resolvents are stored in a set and periodically saved back to the database once a threshold is reached (this is done to minimize I/O bottlenecks). If a thread generates an empty clause, it immediately notifies the `MultiThreadedResolver` by calling the `solutionWasFound()` method, which sets the atomic flag and triggers the system-wide shutdown.

=== Database

For the database, I chose SQLite for its simplicity, my own familiarity, serverless, and its lightweight nature. This is perfect for a self-contained local application. It allows for the project to easily overcome the primary challenge of concurrent databases accessed from multiple threads all while not requiring a separate server. A lot of this is thanks to SQLite's Write-Ahead Logging (WAL) mode. WAL allows multiple threads to read access to the database simultaneously even while another thread is writing to it. This is perfect as resolution theorem proving is a very read-heavy, write-light algorithm. This alone gave a huge performance boost.
...
While WAL handles file-level locking by the SQLite server, it doesn't completely resolve all race conditions caused by the surrounding program. To solve this, I implemented a much more fine-grained concurrency control strategy at the application level to manage the workflow of the `ProverThread`'s request. The `Database` class uses Java's concurrency utilities to create a producer-consumer pattern. A single `ReentrantLock` protects all read and write operations, guaranteeing that database transactions (like adding a batch of new resolvents) are atomic. Furthermore, a `ConAdd configurable `n` example generation and benchmarking output to `Benchmark`

- Introduced `nSizedExample` method for dynamic clause chain generation.
- Enhanced `Benchmark` to compute and display single-threaded vs multi-threaded performance results.
- Added `Results` class to encapsulate timing data and comparison logic.dition` variable (`hasNewClauses`) is paired with the lock. This allows the worker threads to wait efficiently for new work without the need for constantly polling the database for new unresolved clauses. When a `ProverThread` requests new clauses and finds none, it calls `await()` on the condition, putting it to sleep without consuming CPU cycles. When another thread adds new resolvents to the database, it calls `signalAll()` on the condition, waking up all waiting threads to resume processing.
...
The simplest example of this is `CLAUSE_BATCH_SIZE`. It can't be too large, because large gaps between update times cause the other threads to starve, but it can't be too small or the overhead of requesting from the database causes a slowdown. `UNRESOLVED_BATCH_SIZE` comes with the added complexity of not being able to fully know whether or not the clauses we're going to add are truly going to be unique among the existing dataset, because constantly querying to check for uniqueness is inefficient and we can't be constantly checking the other `ProverThread`'s work. This was an incredibly difficult problem to troubleshoot as improperly set constants were incredibly slow, and could easily be confused for a bug. This led to the whole experience feeling like dealing with a heisenbug.
...
I feel overall really content with my work. I was able to implement a database-attached, multi-processed resolution theorem prover, as well as creating a benchmark for RTP performance. It may have taken 90 odd hours, but it was well worth it. It significantly improves the scalability of resolution theorem provers, demonstrating speed ups of 70x in some cases. While this system undoubtedly already exists, it was thrilling to get in the weeds of building a symbolic AI system.
